...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('search-input').addEventListener('keypress', (e) => {
        if (e.key === 'Enter') {
            ...
            if (query) {
                if (query.match(/^https?:\/\//) || query.match(/^[a-z0-9-]+\.[a-z]{2,}/i)) {
    ...
    // Layout edit toggle
    document.getElementById('layout-toggle').addEventListener('click', toggleLayoutEdit);
...
function updateTime() {
    ...
    document.getElementById('month-name').textContent = monthNames[now.getMonth()];
...
async function handleWallpaperUpload(event) {
    ...
    if (!file) return;
    try { await saveWallpaperToDB(file); loadWallpaper(); }
...
async function loadWallpaper() {
    ...
    } catch (err) { console.error("Wallpaper load error:", err); }
}
...
function setupWallpaperSliders() {
    ...
    zoomSlider.addEventListener('input', () => { document.getElementById('zoom-value').textContent = zoomSlider.value + '%'; saveAndApplyWallpaper(); });
    panXSlider.addEventListener('input', () => { document.getElementById('pan-x-value').textContent = panXSlider.value + '%'; saveAndApplyWallpaper(); });
    panYSlider.addEventListener('input', () => { document.getElementById('pan-y-value').textContent = panYSlider.value + '%'; saveAndApplyWallpaper(); });
...
function loadShortcuts() {
    ...
    shortcuts.forEach((site, index) => {
        ...
        let domain = site.url;
        try { domain = new URL(site.url).origin; } catch (e) { }
        ...
        `;
        div.addEventListener('click', (e) => { if (!e.target.closest('.shortcut-actions')) window.location.href = site.url; });
        div.querySelector('.edit-btn').addEventListener('click', (e) => { e.stopPropagation(); openShortcutModal(index, site); });
        div.querySelector('.del-btn').addEventListener('click', (e) => { e.stopPropagation(); if (confirm(`Delete "${site.name}"?`)) deleteShortcut(index); });
...
function openShortcutModal(index = -1, site = null) {
    ...
    if (index >= 0 && site) { title.textContent = "Edit Shortcut"; nameInput.value = site.name; urlInput.value = site.url; }
    else { title.textContent = "Add Shortcut"; nameInput.value = ''; urlInput.value = ''; }
...
function saveShortcut() {
    ...
    const shortcuts = JSON.parse(localStorage.getItem('shortcuts')) || [];
    if (editingIndex >= 0) { shortcuts[editingIndex] = { name, url }; }
...
function loadWidgetPreferences() {
    const prefs = JSON.parse(localStorage.getItem('widgetPreferences')) || DEFAULT_WIDGETS;
...
function handleWidgetToggle(e) {
    ...
    if (widget) {
        if (enabled) { widget.classList.remove('widget-hidden'); if (target === 'weather') loadWeather(); }
...
function updateThemeIcons(theme) {
    ...
    if (theme === 'dark') { darkIcon.classList.remove('hidden'); lightIcon.classList.add('hidden'); }
...
async function resetAll() {
    if (!confirm('Reset everything to default? This will clear wallpaper, shortcuts, to-dos, and all settings.')) return;
    ...
    try { await clearWallpaperDB(); } catch (e) { }
    window.location.reload();
}
...
async function loadWeather() {
    ...
    try {
        ...
        let city = '';
        try { city = data.timezone ? data.timezone.split('/').pop().replace(/_/g, ' ') : ''; } catch (e) { }
        content.innerHTML = `
        ...
        content.innerHTML = '<p class="weather-loading">Tap 🌡️ to enable local weather.</p>';
    }
}

function getWeatherInfo(code) {
    const map = {
        0: { icon: '☀️', condition: 'Clear Sky' }, 1: { icon: '🌤️', condition: 'Mostly Clear' },
        2: { icon: '⛅', condition: 'Partly Cloudy' }, 3: { icon: '☁️', condition: 'Overcast' },
        45: { icon: '🌫️', condition: 'Foggy' }, 48: { icon: '🌫️', condition: 'Rime Fog' },
        51: { icon: '🌦️', condition: 'Light Drizzle' }, 53: { icon: '🌦️', condition: 'Drizzle' },
        55: { icon: '🌧️', condition: 'Heavy Drizzle' }, 61: { icon: '🌧️', condition: 'Light Rain' },
        63: { icon: '🌧️', condition: 'Rain' }, 65: { icon: '🌧️', condition: 'Heavy Rain' },
        71: { icon: '🌨️', condition: 'Light Snow' }, 73: { icon: '❄️', condition: 'Snow' },
        75: { icon: '❄️', condition: 'Heavy Snow' }, 80: { icon: '🌦️', condition: 'Rain Showers' },
        81: { icon: '🌧️', condition: 'Heavy Showers' }, 82: { icon: '⛈️', condition: 'Violent Showers' },
        95: { icon: '⛈️', condition: 'Thunderstorm' }, 96: { icon: '⛈️', condition: 'Thunderstorm + Hail' },
...
function updateTimerDisplay() {
    document.getElementById('timer-minutes').textContent = String(Math.floor(timerSeconds / 60)).padStart(2, '0');
    document.getElementById('timer-seconds').textContent = String(timerSeconds % 60).padStart(2, '0');
}
...
function startTimer() {
    ...
    timerInterval = setInterval(() => {
        ...
        if (timerSeconds <= 0) {
            ...
            if (Notification.permission === 'granted') new Notification('Focus Timer', { body: 'Session complete! Take a break.' });
